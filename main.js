 ...
 *
 *   (nothing)            live, reading the DfT traffic counts and polling
 *   ?source=snapshot     the saved copy in `data/snapshot`, no download needed
 *
 * When the live download cannot be reached the page opens the saved copy
 * instead and says so at the top, rather than showing an error.
 *
...
(function (root) {
  ...
  const TITLE = 'Road traffic counts across Great Britain, by year and vehicle class';
  ...
  function showError(error) {
    ...
    panel.className = 'loading';
    const title = document.createElement('h1');
    title.textContent = 'The traffic counts could not be loaded';
    const message = document.createElement('p');
    message.className = 'loading-message';
  ...
  async function start() {
    ...
    try {
      ...
      const { buildDashboard, fetchInitial, readSnapshot, startPolling, POLL_MS } = root.DftTrafficDemo;
      ...
      if (mode === 'snapshot') {
        ...
        const update = showProgress('Reading the Department for Transport traffic counts...');
        try {
          const initial = await fetchInitial({ onProgress: update });
          rows = initial.rows;
          meta = { live: true, fetchedAt: Date.now() };
        } catch (liveError) {
          /* The download is out of our hands, so a bad day for it should not be
             a blank page here. The saved copy shows the same dashboard, and the
             masthead says plainly that is what you are looking at. */
          console.warn('[dft demo] the live fetch failed, falling back to the saved copy:', liveError);
          update('The traffic counts could not be reached. Opening the saved copy...', 1);
          const saved = await readSnapshot();
          rows = saved.rows;
      ...
      const timings = {
        ...
        rows: built.detailGrid ? built.detailGrid.rows.count() : 0,
        loaded: rows.length,
      ...
      root.__dftTrafficDemo = Object.assign(built, { meta, timings, ready: true });
      console.log('[dft demo] ready', timings);
    } catch (error) {
      root.__dftTrafficDemo = { ready: false, error: String((error && error.message) || error) };
      showError(error);
    }
  }
